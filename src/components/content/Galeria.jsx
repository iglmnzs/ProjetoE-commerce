...
export default function Galeria({ imagens }) {
  ...
  // const selecionarImagem = (indice) => setImg(indice);

  return (
    <div>
      <img className={styles.imgProduto} src={imagens[img]} /> 
        <div className={styles.imgControl}>
          <BaseButton onClick={passarAnterior}>Anterior</BaseButton>
          {
            imagens.map((imagem, indice) => (
              <img className={styles.imgMinGaleria} onClick={() => setImg(indice)} src={imagem}
                key={`galeria_imagem_${indice}`}/>
            ))
          }
          <BaseButton onClick={passarProxima}>Próxima</BaseButton>
        </div>
    </div>
  )
}